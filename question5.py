...
def coefficient(xmatrix , ymatrix , n):
    ...
    print("\n Y-Y^ = \n", stp)
    print("\n RSS = \n", r_s_s)
    onematrix = nm.ones((7,1), dtype=nm.float)
    onematrix[: , 0] = 1
    ...
    print("\n Varience of beta = \n", var_bet)
def main():
    y = nm.array([40, 50, 50, 70, 65, 65, 80])
    x1 = nm.array([100, 200, 300, 400, 500, 600, 700])
    x2 = nm.array([10, 20, 10, 30, 20, 20, 30])
    xmatrix = nm.zeros((7, 3), dtype=nm.float)
    xmatrix[:, 0] = 1
    xmatrix[:, 1] = x1
    xmatrix[:, 2] = x2
    print("\n X-Matrix = \n", xmatrix)
    ymatrix = nm.zeros((7, 1), dtype=nm.float)
    ymatrix[:, 0] = y
    print("\n Y-Matrix = \n", ymatrix)
    n = nm.size(y)
